    ...
    public class IdentificationActivity : AppCompatActivity
    {
		...
		private async void ExecuteIdentification(UUID[] mFaceIds)
		{
			...
			try
			{
				...
				TrainingStatus trainingStatus = await faceClient.GetPersonGroupTrainingStatus(
				...
				result = await faceClient.Identity(
			...
			RunOnUiThread(() =>
			{
				...
				if (mSucceed)
				{
					...
					if (result != null)
					{
						...
						foreach (IdentifyResult identifyResult in result)
						{
							logString += "Face " + identifyResult.FaceId.ToString() + " is identified as "
									+ (identifyResult.Candidates.Count > 0
                                       ? ((Candidate)identifyResult.Candidates[0]).PersonId.ToString()
											: "Unknown Person")
									+ ". ";
						}
						AddLog(logString);
		...
		private class FaceListAdapter : BaseAdapter
		{
			...
			private List<IdentifyResult> mIdentifyResults;
