...
namespace com.rcervantes.xamarinfaceapi_droid.ui
{
    ...
    public class FindSimilarFaceActivity : AppCompatActivity
    {
        ...
        private void SetUiAfterFindPersonalSimilarFaces(SimilarFace[] result)
        {
        ...
        private void SetUiAfterDetectionForAddFace(Face[] result)
        {
            SetAllButtonsEnabledStatus(true);
        ...
        private void Find_Similar_Faces_Click(object sender, EventArgs e)
        {
            ...
            faceIds.Add(mFaceId);
            foreach (Face face in mFaceListAdapter.faces)
            {
            ...
            RunOnUiThread(() =>
            {
                if (mSucceed)
                {
                    String resultString = "Found "
                        + (faces == null ? "0" : faces.Count().ToString())
                            + " matchPerson similar face" + ((faces != null && faces.Count() != 1) ? "s" : "");
        ...
        private async void ExecuteFindFacialSimilarFace(UUID[] mFaceIds)
        {
            ...
            try
            {
                ...
                faces = await faceClient.FindSimilar(
                        ...
                        faceIds,    /*candidate faces */
                        4, /*max number of candidate returned*/
                        FaceServiceClientFindSimilarMatchMode.MatchFace
                        );
            }
            catch (Java.Lang.Exception e)
            {
            ...
            RunOnUiThread(() =>
            {
                if (mSucceed)
                {
                    ...
                    AddLog("Response: Success. " + resultString);
                    AppendInfo((faces == null ? "0" : faces.Count().ToString())
                            + " matchFace similar face" + ((faces != null && faces.Count() != 1) ? "s" : ""));
                }
        ...
        {
            private List<SimilarFace> similarFaces;
            private FindSimilarFaceActivity activity;

            public SimilarFaceListAdapter(SimilarFace[] findSimilarFaceResult, FindSimilarFaceActivity act)
            {
                ...
                {
                    similarFaces = new List<SimilarFace>();
                }
                activity = act;
            }
